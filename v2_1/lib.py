...
#     策略：
#     1. 获取选中的面。
#     2. 提取面的所有边线，并调用 Select2 显式选中它们 (Mark=1)。
#     3. 调用 FeatureFillet (v1) 对选中的边线进行操作。
#     """
...
#     # 获取第一个选中对象
#     selected_obj = sel_mgr.GetSelectedObject6(1, -1)
#     obj_type = sel_mgr.GetSelectedObjectType3(1, -1)
    
#     # swSelFACES = 2
#     if obj_type == 2: 
#         print("   👉 检测到选中了 '面'，正在转换为 '边线'...")
        
#         # 1. 获取边线对象 (不加括号)
#         edges = selected_obj.GetEdges
#         if not edges:
#             print("   ❌ 无法获取边线。")
#             return False
            
#         print(f"      找到 {len(edges)} 条边线，正在执行选择...")
        
#         # 2. 清除原有选择 (非常重要！否则 FeatureFillet 会困惑)
#         swModel.ClearSelection2(True)
        
#         # 3. 显式选中所有边线
#         select_count = 0
#         for edge in edges:
#             # Select2(Append=True, Mark=1)
#             # Mark=1 是倒圆角API识别的关键
#             if edge.Select2(True, 1):
#                 select_count += 1
        
#         print(f"      ✅ 已选中 {select_count} 条边线 (Mark=1)。")
...
#     else:
#         print(f"   👉 当前选中类型代码: {obj_type} (保持原样)")

#     try:
#         print("   👉 正在调用 FeatureFillet (v1)...")
#         # 参数说明：
#         # 1: ConstRadius
#         # radius_m: 半径
#         # 0: Propagate (我们已经手动选中了所有边，不需要自动蔓延，设为0更稳定)
#         # 0, 0, 0, 0: 其他参数传 0 代表 Default/Null
        
#         feat = swModel.FeatureManager.FeatureFillet(
#             1, 
#             float(radius_m), 
#             0, 
#             0, 
...
#         if feat:
#             print(f"   ✅ 圆角特征 '{feat.Name}' 创建成功！")
#             swModel.ClearSelection2(True)
#             return True
#         else:
#             print("   ⚠️ FeatureFillet 返回空。尝试开启 Propagate 再试一次...")
#             # 备用方案：开启自动蔓延
#             feat_retry = swModel.FeatureManager.FeatureFillet(
#                 1, float(radius_m), 1, 0, 0, 0, 0
#             )
#             if feat_retry:
#                 print(f"   ✅ (重试成功) 圆角特征 '{feat_retry.Name}' 创建成功！")
#                 swModel.ClearSelection2(True)
#                 return True
...
# # ... (保留你原有的代码) ...
# '''倒角函数'''
# def run_dynamic_vba_macro(swModel, vba_code: str):
#     """
#     核心工具：将 VBA 代码字符串保存为临时文件并立即运行。
#     [终极修复版] 
#     1. 解决了 (-2147221021) '操作无法使用' 错误：增加 Dispatch 强力回退机制。
#     2. 解决了 RunMacro2 类型不匹配问题 (参数传 0)。
#     """
#     swApp = None
    
#     # --- 1. 获取 SolidWorks 应用程序实例 (三层保险) ---
#     # 尝试 A: 标准获取
#     try:
#         swApp = win32.GetActiveObject("SldWorks.Application")
#     except Exception as e1:
#         # print(f"   ⚠️ GetActiveObject 失败 ({e1})，尝试 Dispatch...")
        
#         # 尝试 B: Dispatch 强制连接 (专门解决 0x800401E3 错误)
#         try:
#             swApp = win32.Dispatch("SldWorks.Application")
#         except Exception as e2:
#             print(f"   ❌ Dispatch 连接也失败: {e2}")

#     # 尝试 C: 如果 swModel 本身有 GetApplication (通常没有，但为了兼容性保留)
#     if not swApp:
#         try:
#             # 注意：大部分 swModel 对象没有这个方法，这里只是最后挣扎
#             swApp = swModel.GetApplication() 
#         except:
#             pass

#     # --- 2. 最终检查 ---
#     if not swApp:
#         print("   ❌ 严重错误: 无法获取 SolidWorks 应用程序对象，宏无法执行。")
#         return False

#     try:
#         # --- 3. 定义临时宏文件路径 ---
#         # 确保路径没有中文，temp 目录通常最安全
#         macro_path = os.path.join(os.getcwd(), "temp_agent_macro.swb")
        
#         # --- 4. 写入 VBA 代码 (编码 cp1252) ---
#         with open(macro_path, "w", encoding="cp1252") as f:
#             f.write(vba_code)
            
#         # --- 5. 运行宏 ---
#         # RunMacro2 参数: FileName, ModuleName, ProcedureName, Options, Error
#         # ⚠️ 必须传 0 (整数) 作为 Error 参数，绝对不能传 ARG_NOTHING
#         swApp.RunMacro2(macro_path, "tempmacro", "main", 0, 0)
        
#         return True
        
#     except Exception as e:
#         print(f"   ❌ 宏执行过程发生异常: {e}")
#         # 如果是权限拒绝，可能是临时文件被占用，稍等再试
#         return False
...
#     [动态宏版] 倒圆角工具。
#     直接在选中的边线或面上生成恒定半径圆角。
#     修复了编码问题：移除了 VBA 字符串中的中文注释。
#     """
#     print(f"\n--- ✨ 正在执行倒圆角 (VBA代理): R={radius_m}m ---")
    
#     # 1. 检查是否有选中项
#     sel_mgr = swModel.SelectionManager
#     if sel_mgr.GetSelectedObjectCount2(-1) == 0:
#         print("   ❌ 错误：未选中任何边线或面，无法倒角。")
#         return False

#     # 2. 构建 VBA 代码 (⚠️关键修复：全部使用英文注释，避免 cp1252 编码报错)
#     vba_script = f"""
# Dim swApp As Object
# Dim Part As Object
...
#     ' Force unit to meters (Python input is already in meters)
#     Dim myRadius As Double
#     myRadius = {radius_m}
    
#     ' Execute Fillet
#     ' Parameters: Option=1 (ConstRadius), Radius=myRadius, ConstOptions=1 (Propagate)
    
#     Dim fMgr As Object
#     Set fMgr = Part.FeatureManager
    
#     ' Try creating fillet with Tangent Propagation (1)
#     Dim swFeat As Object
#     Set swFeat = fMgr.FeatureFillet(1, myRadius, 1, 0, 0, 0, 0)
    
#     If swFeat Is Nothing Then
#         ' Retry without propagation (0) if the first attempt failed
#         Set swFeat = fMgr.FeatureFillet(1, myRadius, 0, 0, 0, 0, 0)
#     End If
...
#     # 4. 执行宏 (确保 run_dynamic_vba_macro 已修复为 GetActiveObject 版本)
#     run_dynamic_vba_macro(swModel, vba_script)
...
#         return True
#     else:
#         print("   ❌ 圆角创建失败 (VBA执行完成但未生成特征)。")
#         print("      可能原因：半径过大导致几何体消失，或选中项不适合倒角。")
#         return False