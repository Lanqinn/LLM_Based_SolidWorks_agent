...
def extrude_cylinder(swModel, radius_m: float, height_m: float, center_x_m: float = 0.0, center_y_m: float = 0.0):
    """
    在指定面上创建一个圆柱体（拉伸实体）。优先在当前选中的面上创建；如果没有选中，则在 上视基准面 上创建
    所有输入参数均使用米 (m) 作为单位。
    ...
    if sel_mgr.GetSelectedObjectCount2(-1) > 0:
        ...
        print(f"\n--- 🌐 未检测到选择，将在【上视基准面】上创建圆柱：R={radius_m}m, H={height_m}m ---")
        swModel.ClearSelection2(True)
        swModel.Extension.SelectByID2("上视基准面", "PLANE", 0, 0, 0, False, 0, ARG_NOTHING, 0)
...
'''function: 截取当前视图'''
def capture_viewport_screenshot(swModel, save_path: str):
    """
    将当前 SolidWorks 视图保存为 JPG 图片。
    Args:
        swModel: SolidWorks 模型对象
        save_path: 图片保存的绝对路径 (建议以 .jpg 结尾)
    """
    try:
        print(f"   📷 正在截取当前视图: {save_path} ...")

        # --- 1. 视图优化 (新增) ---
        # 强制切换到 "等轴测" (Isometric) 视图，这样顶面、前面、右面都能看到
        # 参数 7 代表 Isometric View
        swModel.ShowNamedView2("*Isometric", 7)
        
        # 1. 调整视图：全屏显示，保证模型在画面中心
        swModel.ViewZoomtofit2() 
        ...
        # 3. 执行保存 (SaveAs3)
        # 参数: Name, SaveAsCopy, Options
        # 返回值: 0 代表成功 (不同版本可能略有不同，建议结合文件检查)
        result = swModel.SaveAs3(save_path, 0, 0)
