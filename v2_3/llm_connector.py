...
# --- 1. 选面工具 (升级版：支持多选) ---
    ...
    # --- 2. 圆柱拉伸 ---
    {
        ...
        "function": {
            ...
            "description": "创建圆柱体。逻辑：如果在调用此工具前已经选中了一个面，将在该面上生成；否则默认在 上视基准面 生成。",
            "parameters": {
                "type": "object",
                "properties": {
                    "radius_m": {
    ...
    # --- 3. 圆孔切除 ---
    {
        ...
        "function": {
            ...
            "description": "切除圆形孔。逻辑：如果在调用此工具前已经选中了一个面，将在该面上打孔；否则默认在 上视基准面打孔。",
            "parameters": {
                "type": "object",
                "properties": {
                    "radius_m": {
    ...
    # --- 4. 长方体拉伸 ---
    {
            ...
            "function": {
                ...
                "description": "通用长方体拉伸工具。建模逻辑：1. height_m 是拉伸方向的长度（如立柱的高度）。2. 若之前调用过 select_face，则 center_x/y 是相对于该面中心的局部坐标；若未选面，则是在 上视基准面 上的世界坐标。",
                "parameters": {
                    "type": "object",
                    "properties": {
    ...
    # --- 5. 矩形切除 ---
    {
        ...
        "function": {
            ...
            "description": "切除矩形孔。逻辑：如果在调用此工具前已经选中了一个面，将在该面上切除；否则默认在上视基准面 切除。",
            "parameters": {
                "type": "object",
                "properties": {
                    "length_m": {
    ...
    # --- 21. [新增] 屏幕截图工具 ---
    {
        ...
        "function": {
            ...
            "description": "截取当前 SolidWorks 3D 视图的屏幕截图。通常在完成关键建模步骤后调用，用于视觉记录或后续的视觉检查。",
            "parameters": {
                "type": "object",
                "properties": {
                    "save_path": {
                        "type": "string",
                        "description": "图片保存的完整绝对路径（必须以 .jpg 结尾）。如果不确定，可以使用与零件文件相同的目录。"
                    }
                },
                "required": ["save_path"]
            }
    ...
    # --- 22. [新增] 视觉自我验收工具 ---
    {
        "type": "function",
        "function": {
            "name": "analyze_modeling_result",
            "description": "【视觉大脑】调用视觉大模型对当前的建模结果进行验收。Agent 会将用户指令和当前截图发送给 Vision LLM，并获得“通过/失败”的反馈报告。",
            "parameters": {
                "type": "object",
                "properties": {
                    "user_command": {
                        "type": "string",
                        "description": "用户的原始自然语言需求（例如：‘建立一个带孔的红砖’）。"
                    },
                    "image_path": {
                        "type": "string",
                        "description": "用于分析的截图路径（通常是上一步 capture_viewport_screenshot 保存的路径）。"
                    }
                },
                "required": ["user_command", "image_path"]
            }
        }
    }
...
# 1. 确保 lib 被导入，因为要用它的 encode 函数
import lib 
# 2. 配置视觉模型名称 (根据你的 API 供应商调整)
# 硅基流动推荐: "Qwen/Qwen2-VL-72B-Instruct" 
# 智谱推荐: "glm-4v"
VISION_MODEL_NAME = "Qwen/Qwen2-VL-72B-Instruct" 

def analyze_modeling_result(swModel, user_command: str, image_path: str):
    """
    视觉验收函数：将‘用户需求’和‘SW截图’发给大模型，让它评价做得对不对。
    """
    print("\n--- 👁️ 正在调用视觉大模型进行验收... ---")
    
    # 1. 编码图片
    base64_image = lib.encode_image_to_base64(image_path)
    if not base64_image:
        return "图片加载失败，无法分析。"

    # 2. 构造质检员的 System Prompt
    system_instruction = """
    你是一个 SolidWorks 建模质检专家。
    你的任务是对比【用户的自然语言指令】和【最终建模结果的截图】。
    
    请检查以下几点：
    1. **完整性**：用户要求的特征（如圆孔、文字、倒角）是否都在？
    2. **几何正确性**：形状是否正确？（例如圆柱是否变成了长方体？）
    3. **布局**：位置大概对不对？（例如孔是否打偏了？）
    
    请给出简短的评价，如果发现明显错误，请指出。
    """
            ...
            messages=[
                ...
                },
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": f"用户指令是：'{user_command}'。\n这是 Agent 建模后的结果截图，请检查是否合格："
                        },
                        {
                            "type": "image_url",
                            "image_url": {
...
FUNCTION_MAP = {
    ...
    # 视觉相关映射
    "capture_viewport_screenshot": lib.capture_viewport_screenshot,
    "analyze_modeling_result": analyze_modeling_result, # 映射到本文件定义的函数
